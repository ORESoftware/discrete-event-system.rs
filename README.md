...
| Vehicle routing and routing heuristics | `classical_optimization_models`; Clarke-Wright savings and nearest-neighbor CVRP heuristics, exact small-instance CVRP route-cover dynamic programming, and same-input OR-Tools Routing/reference cross-checks |
| Scheduling and planning | `classical_optimization_models`, `external_scheduling_reference`; job-shop dispatch heuristics, exact small-instance job-shop branch-and-bound, flow-shop NEH sequencing, and same-input Python exact / OR-Tools CP-SAT scheduling cross-checks |
