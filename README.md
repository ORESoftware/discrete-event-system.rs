...
| Solver-style modeling facade | `math_program`; named-variable LP/MIP facade cross-checked against external HiGHS, GLPK, SCIP, CBC, OR-Tools GLOP/CP-SAT, optional commercial Gurobi/CPLEX/Xpress API oracles, and optional commercial Gurobi/CPLEX/Xpress/LINDO CLI oracles when locally available, with CPLEX-LP and MPS text export for local solver CLIs, including `<=`/`>=`/`=`/range rows, objective offsets, bounds, integer/binary domains, indicator lowering, lazy-row materialization for external validation, external MIP start/limit/gap/search-control pass-through, max/general-constraint lowering, conflict refinement, feasibility relaxation, and solution pools |
