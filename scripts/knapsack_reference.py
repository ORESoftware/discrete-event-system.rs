#!/usr/bin/env python3
"""Thin compatibility launcher for the Rust knapsack reference."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="auto",
        metavar="SOLVER",
        help="solver alias to pass through to the Rust knapsack_reference binary",
    )
    args = parser.parse_args()
    try:
        exec_rust_reference(args.solver)
    except Exception as exc:
        print(json.dumps(result("error", "rust:knapsack-reference", str(exc))))
        return 1
    return 0
