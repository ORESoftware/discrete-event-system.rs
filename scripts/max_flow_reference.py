#!/usr/bin/env python3
"""Thin compatibility launcher for the Rust max-flow reference."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="auto",
        metavar="SOLVER",
        help="solver alias to pass through to the Rust max_flow_reference binary",
    )
    args = parser.parse_args()
    try:
        exec_rust_reference(args.solver)
    except Exception as exc:
        print(json.dumps(result("error", "rust:max-flow-reference", str(exc))))
        return 1
    return 0
