...
RUST_REFERENCE_SOLVERS = ("auto", "fallback", "rust-edmonds-karp", "rust-exact")
...
def local_rust_binary_is_current(repo_root: str, binary_path: str) -> bool:
    ...
    source_paths = [
        os.path.join(repo_root, "src", "bin", "max_flow_reference.rs"),
        os.path.join(repo_root, "src", "des", "general", "external_max_flow_reference.rs"),
        os.path.join(repo_root, "src", "des", "general", "max_flow.rs"),
    ]
    return all(
...
def external_rust_fallback_enabled() -> bool:
    value = os.environ.get("MAX_FLOW_REFERENCE_EXTERNAL_FALLBACK", "")
    return value.strip().lower() in ("1", "true", "yes", "on", "rust")


def external_rust_first_enabled() -> bool:
    values = (
        os.environ.get("MAX_FLOW_REFERENCE_RUST_FIRST", ""),
...
def main() -> int:
    ...
    if external_rust_first_enabled() and args.solver == "ortools":
        os.environ["MAX_FLOW_REFERENCE_EXTERNAL_FALLBACK"] = "rust"
        exec_rust_reference(args.solver)
    if (
