#!/usr/bin/env python3
"""Thin compatibility launcher for the Rust quadratic reference."""
...
def status_payload(status: str, solver: str, message: str) -> dict[str, object]:
    return {
        "status": status,
        "solver": solver,
        "x": [],
...
def exec_rust_reference(solver: str, max_enumerations: int) -> None:
...
def main() -> int:
    ...
    try:
        exec_rust_reference(args.solver, args.max_enumerations)
