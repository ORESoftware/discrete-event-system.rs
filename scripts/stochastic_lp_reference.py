...
def main() -> int:
    ...
    parser.add_argument("--solver", choices=["auto", "scipy", "fallback"], default="auto")
    ...
    try:
        ...
        if args.solver == "fallback":
            output = result(
                "unsupported",
                "fallback:unsupported-continuous-slp",
                message="continuous stochastic LP fallback is not implemented; use scipy",
            )
        else:
