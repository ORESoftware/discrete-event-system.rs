#!/usr/bin/env python3
"""Thin compatibility launcher for the Rust weighted Max-SAT reference."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="auto",
        metavar="SOLVER",
        help="solver alias to pass through to the Rust weighted_max_sat_reference binary",
    )
    ...
    try:
        ...
        print(
            json.dumps(
                result(
                    "error",
                    "rust:weighted-max-sat-reference",
                    str(exc),
                )
            )
        )
