...
//! TypeScript source: `src/des/main-factmachine.ts`
//! Rust target: `src/bin/main_factmachine.rs`
...
pub const MIGRATION: MigrationFile = MigrationFile::scaffolded(
    "src/des/main-factmachine.ts",
    "src/bin/main_factmachine.rs",
    &["RUST MIGRATION: target src/bin/main_factmachine.rs.", "RUST MIGRATION: Keep this binary thin: parse CLI/env/path inputs with clap/std::env/PathBuf, then call library orchestration.", "RUST MIGRATION: Port the runnable body as fn main() -> Result<()> and move reusable DES setup into src/des modules/traits.", "RUST MIGRATION: Keep JSON examples/config as serde-deserialized structs instead of ad-hoc JS objects."],
    &["Bettor", "FactMachineParams", "FactMachineResult", "LMSR", "LMSROptions", "defaultParams", "liquidityToB", "outcomeMatrix", "runFactMachine"],
);
