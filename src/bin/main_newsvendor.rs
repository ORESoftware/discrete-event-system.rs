...
//! TypeScript source: `src/des/main-newsvendor.ts`
//! Rust target: `src/bin/main_newsvendor.rs`
...
pub const MIGRATION: MigrationFile = MigrationFile::scaffolded(
    "src/des/main-newsvendor.ts",
    "src/bin/main_newsvendor.rs",
    &["RUST MIGRATION: target src/bin/main_newsvendor.rs.", "RUST MIGRATION: Keep this binary thin: parse CLI/env/path inputs with clap/std::env/PathBuf, then call library orchestration.", "RUST MIGRATION: Port the runnable body as fn main() -> Result<()> and move reusable DES setup into src/des modules/traits.", "RUST MIGRATION: Keep JSON examples/config as serde-deserialized structs instead of ad-hoc JS objects."],
    &["DemandDist", "NewsvendorParams", "analyticalOptimalQ", "bruteSearchOptimalQ", "cdfFromPMF", "demandPoissonPMF", "demandUniformPMF", "expectedProfit", "mdpOptimalQ", "meanFromDemand", "newsvendorMDPSpec", "profit", "sampleDemand", "simulate"],
);
