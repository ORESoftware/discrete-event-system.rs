...
mod tests {
    ...
    static SET_COVER_CLI_ENV_LOCK: Mutex<()> = Mutex::new(());
    ...
    fn ortools_cli_alias_defaults_to_rust_reference_without_python() {
        let _lock = SET_COVER_CLI_ENV_LOCK
            .lock()
            .expect("lock set-cover CLI env guard");
        let _force_python_guards = set_cover_force_python_off_guards();
        let _python_bin_guard =
            EnvVarGuard::set("PYTHON_BIN", "/definitely/not-python-for-set-cover-cli");
        let _python_guard = EnvVarGuard::set("PYTHON", "/definitely/not-python-for-set-cover-cli");

        let output = run(
            vec![
                "set_cover_reference".to_string(),
                "--solver=ortools:cp-sat-set-cover".to_string(),
            ],
            SAMPLE,
        )
        .expect("run");
