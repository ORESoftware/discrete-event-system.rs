...
<main>
  ...
  <div class="controls">
    ...
    <button id="reverse" title="Play backward in time (R)" aria-pressed="false">Reverse</button>
...
<footer>
  ...
  Press space to play/pause, &larr;/&rarr; to step, R to reverse.
</footer>
...
(function() {
  ...
  function tick(ts) {
    ...
    if (advance >= 1) {
      // Step in the current time direction; clamp to [0, N-1].
      i = Math.min(N - 1, Math.max(0, i + dir * Math.floor(advance)));
      lastTimestamp = ts;
      render(i);
      // Stop once we reach whichever end we are travelling toward.
      if ((dir > 0 && i >= N - 1) || (dir < 0 && i <= 0)) {
        playing = false; playBtn.textContent = 'Play'; lastTimestamp = null; return;
      }
    }
  ...
  function setPlaying(p) {
    ...
    if (playing) {
      // If parked at the terminal frame for the current direction, wrap to the
      // far end so Play always makes progress (forward from start, reverse from end).
      if (dir > 0 && i >= N - 1) i = 0;
      else if (dir < 0 && i <= 0) i = N - 1;
      lastTimestamp = null;
      requestAnimationFrame(tick);
    }
  }
  ...
  function setDirection(d) {
    dir = d < 0 ? -1 : 1;
    reverseBtn.setAttribute('aria-pressed', dir < 0 ? 'true' : 'false');
    reverseBtn.textContent = dir < 0 ? 'Reverse \u2713' : 'Reverse';
    // Reset the frame-timer so an in-flight playback continues smoothly in the
    // newly selected time direction.
  ...
  reverseBtn.addEventListener('click', function() { setDirection(-dir); });
...
mod tests {
    ...
    #[test]
    fn build_html_has_reverse_and_speed_controls() {
        let html = build_html(&tiny_anim());
        // Reverse-time control is present and wired to a direction toggle.
        assert!(html.contains("id=\"reverse\""));
        assert!(html.contains("function setDirection"));
        assert!(html.contains("reverseBtn.addEventListener"));
        // Playback walks the pre-collected index in the selected direction,
        // which is only possible because all frames are recorded up-front.
        assert!(html.contains("i + dir * Math.floor(advance)"));
        assert!(html.contains("dir < 0 && i <= 0"));
        // A speed control offers a range of playback rates (0.25x .. 16x).
        assert!(html.contains("id=\"speed\""));
        assert!(html.contains(">16x<"));
    }
