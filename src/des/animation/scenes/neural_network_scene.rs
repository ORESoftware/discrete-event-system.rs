...
pub fn build_neural_xor_animation(
    result: &SupervisedNeuralNetDESResult,
    ...
    for i in 0..num_frames {
        let step = (total.saturating_sub(1)).min(
            ((i as f64 * (total as f64 - 1.0)) / (1.0_f64.max(num_frames as f64 - 1.0))).round()
                as usize,
        );
...
fn build_xor_frame(
    ...
    for li in 0..dims.len() {
        let n = dims[li];
        let ys: Vec<f64> = (0..n)
            .map(|j| y0 + (j as f64 + 1.0) * ((y1 - y0) / (n as f64 + 1.0)))
            .collect();
    ...
    shapes.push(Shape::Circle(CircleShape {
        ...
        y: 72.0,
        r: 8.0,
        fill: c::PURPLE.to_string(),
        title: Some("sample token moving across layer stations".to_string()),
        ..Default::default()
    }));
    shapes.push(Shape::Line(LineShape {
        x1: x0,
    ...
    for li in 0..node_pos.len() {
        for j in 0..node_pos[li].len() {
            ...
            shapes.push(Shape::Circle(CircleShape {
                ...
                stroke: Some(if li == node_pos.len() - 1 {
                    c::BLUE.to_string()
                } else {
                    "#475569".to_string()
                }),
    ...
    metric_panel(
        ...
        &[
            (
                "sample".to_string(),
                format!("{} -> {}", sample.label, js_num(sample.y)),
            ),
            (
                "training step".to_string(),
                format!("{} / {}", step + 1, total),
            ),
            (
                "progress".to_string(),
                format!("{}%", to_fixed(100.0 * progress, 1)),
            ),
    ...
    shapes.push(Shape::Text(TextShape {
        x: 88.0,
        y: 468.0,
        text: "final predictions".to_string(),
        font_size: Some(13.0),
        fill: Some(c::INK.to_string()),
        font_weight: Some(FontWeight::Bold),
        ..Default::default()
    }));
    ...
    for i in 0..final_pred.len() {
        ...
        shapes.push(Shape::Text(TextShape {
            x: x + 38.0,
            y: y + 124.0,
            text: bar_labels.get(i).copied().unwrap_or("").to_string(),
            anchor: Some(Anchor::Middle),
            font_size: Some(12.0),
            fill: Some(c::MUTED.to_string()),
            ..Default::default()
    ...
    vec![
        ...
        ChartSpec {
            ...
            series: vec![ChartSeries {
                ...
                color: c::GREEN.to_string(),
                t: pred_t,
                y: result.predictions.iter().map(|v| v[0]).collect(),
            }],
            cursor: Some(false),
            ..Default::default()
        },
...
pub fn build_neural_q_corridor_animation(
...
fn build_corridor_frame(
    ...
    progress_bar(
        ...
        280.0,
        14.0,
        step as f64 / 1.0_f64.max(path.len() as f64 - 1.0),
        c::GREEN,
    );
    let policy_str: Vec<String> = result.policy.iter().map(|p| p.to_string()).collect();
    let caption = format!(
        "greedy rollout step={} state={} policy=[{}]",
        step,
        state,
        policy_str.join(", ")
    );
...
fn build_q_charts(result: &NeuralQLearningResult) -> Vec<ChartSpec> {
    let episodes: Vec<f64> = (0..result.reward_history.len())
        .map(|i| (i + 1) as f64)
        .collect();
    vec![
        ChartSpec {
            ...
            series: vec![ChartSeries {
                ...
                color: c::BLUE.to_string(),
                t: episodes,
                y: result.reward_history.clone(),
            }],
            ..Default::default()
        },
        ChartSpec {
            ...
            series: vec![ChartSeries {
                label: "loss".to_string(),
                color: c::RED.to_string(),
                t: (0..result.loss_history.len())
                    .map(|i| (i + 1) as f64)
                    .collect(),
                y: result.loss_history.clone(),
            }],
            ..Default::default()
        },
    ]
...
pub fn build_neural_ode_animation(
    trace: &ODETrace,
    rate: f64,
    exact_final: f64,
    error: f64,
    ...
    let charts = vec![ChartSpec {
        ...
        series: vec![
            ChartSeries {
                label: "network RK4".to_string(),
                color: c::BLUE.to_string(),
                t: trace.t.clone(),
                y: y.clone(),
            },
            ChartSeries {
                label: "exact exp decay".to_string(),
                color: c::GREEN.to_string(),
                t: trace.t.clone(),
                y: exact,
            },
...
fn build_ode_frame(
    ...
    let mut shapes = base_background(
        "Neural ODE",
        &format!("network vector field dy/dt = -{} y", js_num(rate)),
    );
    let t = trace.t[idx];
    let y = trace.y[idx][0];
    let x0 = 90.0;
    ...
    for i in 0..=idx {
        d += &format!(
            "{} {} {} ",
            if i == 0 { "M" } else { "L" },
            to_fixed(sx(trace.t[i]), 2),
            to_fixed(sy(trace.y[i][0]), 2)
        );
    }
    shapes.push(Shape::Path(PathShape {
        d,
        stroke: Some(c::BLUE.to_string()),
    ...
    shapes.push(Shape::Rect(RectShape {
        ...
        w: 300.0,
        h: 250.0,
        fill: c::PANEL.to_string(),
        stroke: Some(c::GRID.to_string()),
        stroke_width: Some(1.0),
        rx: Some(5.0),
        ..Default::default()
    }));
    shapes.push(Shape::Text(TextShape {
        x: nx,
        y: ny - 10.0,
        text: "vector-field network".to_string(),
        font_size: Some(13.0),
        fill: Some(c::INK.to_string()),
        font_weight: Some(FontWeight::Bold),
        ..Default::default()
