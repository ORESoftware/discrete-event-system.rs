...
pub fn build_warehouse_comparison_frame(result: &WarehouseComparisonResult, frame_index: i64, frames_per_trace_step: i64) -> FrameParts {
    ...
    draw_panel(&mut shapes, &result.baseline, &base_motion, &base_panel, "#b91c1c");
    draw_panel(&mut shapes, &result.track3t, &track_motion, &track_panel, "#047857");
    draw_delta_summary(&mut shapes, result);

    let caption = [
        format!("frame {}", frame_index + 1),
        format!("baseline t={} min", to_fixed(interpolated_time(&base_motion), 1)),
        format!("track3t t={} min", to_fixed(interpolated_time(&track_motion), 1)),
    ]
    .join(" | ");
    FrameParts::with_caption(shapes, caption)
}

pub fn warehouse_comparison_frame_count(result: &WarehouseComparisonResult, frames_per_trace_step: i64) -> i64 {
    (result.baseline.trace.len().max(result.track3t.trace.len())) as i64 * 1.max(frames_per_trace_step)
}

pub fn warehouse_comparison_frame_time(result: &WarehouseComparisonResult, frame_index: i64, frames_per_trace_step: i64) -> f64 {
    let base = interpolated_time(&motion_at(&result.baseline.trace, frame_index, frames_per_trace_step));
    let track = interpolated_time(&motion_at(&result.track3t.trace, frame_index, frames_per_trace_step));
    base.max(track)
}
...
pub fn build_warehouse_comparison_charts(result: &WarehouseComparisonResult) -> Vec<ChartSpec> {
    let misses_max = {
        let mut m = 1.0_f64;
        for r in result.baseline.trace.iter().chain(result.track3t.trace.iter()) {
    ...
    vec![
        ChartSpec {
            ...
            y_min: Some(0.0),
            y_max: Some(result.baseline.metrics.completed_jobs.max(result.track3t.metrics.completed_jobs)),
            series: vec![
                cumulative_series(&result.baseline, "baseline", "#b91c1c", |r| r.cumulative_delivered),
                cumulative_series(&result.track3t, "track3t", "#047857", |r| r.cumulative_delivered),
            ],
            ..Default::default()
        },
        ChartSpec {
            ...
            series: vec![
                cumulative_series(&result.baseline, "baseline", "#b91c1c", |r| r.cumulative_search_misses + r.cumulative_errors),
                cumulative_series(&result.track3t, "track3t", "#047857", |r| r.cumulative_search_misses + r.cumulative_errors),
            ],
            ..Default::default()
        },
    ]
...
fn draw_panel(shapes: &mut Vec<Shape>, scenario_result: &WarehouseScenarioResult, motion: &MotionFrame, g: &PanelGeom, accent: &str) {
    let row = motion.row;
    shapes.push(Shape::Rect(RectShape { x: g.x, y: g.y, w: g.w, h: g.h, fill: "#ffffff".to_string(), stroke: Some("#d1d5db".to_string()), stroke_width: Some(1.0), rx: Some(6.0), ..Default::default() }));
    shapes.push(Shape::Text(TextShape { x: g.x + 18.0, y: g.y + 28.0, text: scenario_result.scenario.label.clone(), font_size: Some(17.0), fill: Some("#111827".to_string()), font_weight: Some(FontWeight::Bold), ..Default::default() }));
    shapes.push(Shape::Text(TextShape { x: g.x + 18.0, y: g.y + 50.0, text: metric_line(scenario_result), font_size: Some(12.0), fill: Some("#4b5563".to_string()), ..Default::default() }));
    ...
    for (i, station) in scenario_result.layout.stations.iter().enumerate() {
        ...
        if belief > 0.01 {
            shapes.push(Shape::Circle(CircleShape {
                x: p.0,
                y: p.1,
                r: (if compact { 9.0 } else { 14.0 }) + (if compact { 22.0 } else { 30.0 }) * belief,
    ...
    for station in &scenario_result.layout.stations {
        ...
        let box_w = if compact { if station.kind == "storage" { 34.0 } else { 52.0 } } else { 68.0 };
        let box_h = if compact { if station.kind == "storage" { 24.0 } else { 30.0 } } else { 34.0 };
        shapes.push(Shape::Rect(RectShape {
            ...
            stroke: Some(if is_destination { accent.to_string() } else { "#374151".to_string() }),
            stroke_width: Some(if is_destination { 3.0 } else { 1.0 }),
            title: Some(format!("{} ({})", station.label, station.kind)),
            ..Default::default()
        }));
        shapes.push(Shape::Text(TextShape { x: p.0, y: p.1 + 4.0, text: station.label.clone(), font_size: Some(if compact { 9.0 } else { 10.0 }), fill: Some("#111827".to_string()), anchor: Some(Anchor::Middle), ..Default::default() }));
    }

    if let Some(row) = row {
        let route = route_path_points(&scenario_result.layout, &row.forklift_before, &row.forklift_after, g);
        ...
        draw_pallet(shapes, pallet.0, pallet.1, row.carrying_before || row.carrying_after || row.event == "pickup" || row.event == "delivered");
        draw_forklift(shapes, forklift.0, forklift.1, accent, row.carrying_before || row.carrying_after, direction_angle(&route, motion.phase));
        shapes.push(Shape::Text(TextShape { x: g.x + 18.0, y: g.y + g.h - 58.0, text: format!("{}: {} via {}", row.job_id, row.event, row.action_target), font_size: Some(12.0), fill: Some("#111827".to_string()), font_weight: Some(FontWeight::Bold), ..Default::default() }));
        shapes.push(Shape::Text(TextShape { x: g.x + 18.0, y: g.y + g.h - 38.0, text: format!("obs: {}", row.observation), font_size: Some(11.0), fill: Some("#4b5563".to_string()), ..Default::default() }));
        shapes.push(Shape::Text(TextShape { x: g.x + 18.0, y: g.y + g.h - 18.0, text: format!("cycle {} min | entropy {} | errors {}", to_fixed(row.cycle_time_so_far, 1), to_fixed(row.belief_entropy, 2), crate::des::animation::types::js_num(row.cumulative_errors)), font_size: Some(11.0), fill: Some("#4b5563".to_string()), ..Default::default() }));
    }
}
...
fn draw_motion_path(shapes: &mut Vec<Shape>, points: &[(f64, f64)], accent: &str) {
    ...
    let d = points
        .iter()
        .enumerate()
        .map(|(i, p)| format!("{} {} {}", if i == 0 { "M" } else { "L" }, to_fixed(p.0, 1), to_fixed(p.1, 1)))
        .collect::<Vec<_>>()
        .join(" ");
    shapes.push(Shape::Path(PathShape { d, stroke: Some(accent.to_string()), stroke_width: Some(2.0), fill: Some("none".to_string()), opacity: Some(0.3), ..Default::default() }));
}

fn draw_forklift(shapes: &mut Vec<Shape>, x: f64, y: f64, accent: &str, loaded: bool, angle: f64) {
    let body_w = 28.0;
    let body_h = 16.0;
    shapes.push(Shape::Rect(RectShape { x: x - body_w / 2.0, y: y - body_h / 2.0 - 20.0, w: body_w, h: body_h, rx: Some(4.0), fill: accent.to_string(), stroke: Some("#111827".to_string()), stroke_width: Some(1.3), title: Some("smart-movable forklift".to_string()), ..Default::default() }));
    shapes.push(Shape::Rect(RectShape { x: x - 4.0, y: y - body_h / 2.0 - 29.0, w: 13.0, h: 10.0, rx: Some(3.0), fill: "#e0f2fe".to_string(), stroke: Some("#0f172a".to_string()), stroke_width: Some(1.0), ..Default::default() }));
    shapes.push(Shape::Circle(CircleShape { x: x - 9.0, y: y - 10.0, r: 3.0, fill: "#111827".to_string(), ..Default::default() }));
    shapes.push(Shape::Circle(CircleShape { x: x + 9.0, y: y - 10.0, r: 3.0, fill: "#111827".to_string(), ..Default::default() }));
    let fork_start_x = x + angle.cos() * 12.0;
    let fork_start_y = y - 20.0 + angle.sin() * 12.0;
    shapes.push(Shape::Line(LineShape { x1: fork_start_x, y1: fork_start_y, x2: fork_start_x + angle.cos() * 18.0, y2: fork_start_y + angle.sin() * 18.0, stroke: "#111827".to_string(), stroke_width: Some(2.0), ..Default::default() }));
    shapes.push(Shape::Line(LineShape { x1: fork_start_x, y1: fork_start_y + 5.0, x2: fork_start_x + angle.cos() * 18.0, y2: fork_start_y + 5.0 + angle.sin() * 18.0, stroke: "#111827".to_string(), stroke_width: Some(2.0), ..Default::default() }));
    shapes.push(Shape::Text(TextShape { x, y: y - 17.0, text: "F".to_string(), font_size: Some(10.0), fill: Some("#ffffff".to_string()), anchor: Some(Anchor::Middle), font_weight: Some(FontWeight::Bold), ..Default::default() }));
    if loaded {
        shapes.push(Shape::Text(TextShape { x: x + 22.0, y: y - 24.0, text: "loaded".to_string(), font_size: Some(10.0), fill: Some(accent.to_string()), font_weight: Some(FontWeight::Bold), ..Default::default() }));
    }
}
...
fn draw_pallet(shapes: &mut Vec<Shape>, x: f64, y: f64, active: bool) {
    shapes.push(Shape::Rect(RectShape {
        ...
        h: 14.0,
        rx: Some(2.0),
        fill: if active { "#f59e0b".to_string() } else { "#fbbf24".to_string() },
...
fn pallet_point(layout: &WarehouseLayout, row: &WarehouseStepTrace, phase: f64, forklift: (f64, f64), g: &PanelGeom) -> (f64, f64) {
    ...
    if !row.carrying_before && row.carrying_after {
        return if phase < 0.78 { (before_p.0, before_p.1 + 24.0) } else { (forklift.0 + 18.0, forklift.1 - 22.0) };
    }
    if row.carrying_before && !row.carrying_after {
        return if phase < 0.86 { (forklift.0 + 18.0, forklift.1 - 22.0) } else { (after_p.0, after_p.1 + 24.0) };
    }
    (before_p.0 + (after_p.0 - before_p.0) * phase, before_p.1 + 24.0 + (after_p.1 - before_p.1) * phase)
}

fn draw_routes(shapes: &mut Vec<Shape>, layout: &WarehouseLayout, g: &PanelGeom) {
    let by_id: HashMap<&str, &StationDefinition> = layout.stations.iter().map(|s| (s.id.as_str(), s)).collect();
    let empty = Vec::new();
    ...
    for (a_id, b_id) in route_pairs {
        ...
        shapes.push(Shape::Line(LineShape { x1: pa.0, y1: pa.1, x2: pb.0, y2: pb.1, stroke: "#cbd5e1".to_string(), stroke_width: Some(4.0), opacity: Some(0.8), ..Default::default() }));
    }
}
...
fn draw_delta_summary(shapes: &mut Vec<Shape>, result: &WarehouseComparisonResult) {
    let x = 34.0;
    let y = 504.0;
    shapes.push(Shape::Rect(RectShape { x, y, w: 1136.0, h: 32.0, fill: "#111827".to_string(), rx: Some(5.0), ..Default::default() }));
    let d = &result.deltas;
...
fn make_panel(x: f64, y: f64, stations: &[StationDefinition]) -> PanelGeom {
    let max_x = stations.iter().map(|s| s.x).fold(f64::NEG_INFINITY, f64::max);
    let max_y = stations.iter().map(|s| s.y).fold(f64::NEG_INFINITY, f64::max);
    PanelGeom { x, y, w: 540.0, h: 398.0, plot_x: x + 58.0, plot_y: y + 82.0, plot_w: 420.0, plot_h: 230.0, max_x, max_y }
}
...
fn motion_at(trace: &[WarehouseStepTrace], frame_index: i64, frames_per_trace_step: i64) -> MotionFrame<'_> {
    if trace.is_empty() {
        return MotionFrame { row: None, row_index: -1, phase: 1.0 };
    }
    ...
    let phase = if subframes <= 1 { 1.0 } else { sub_index as f64 / (subframes as f64 - 1.0) };
    MotionFrame { row: Some(&trace[row_index as usize]), row_index, phase }
}
...
fn route_path_points(layout: &WarehouseLayout, from_id: &str, to_id: &str, g: &PanelGeom) -> Vec<(f64, f64)> {
...
fn row_corridor_path(layout: &WarehouseLayout, from: &StationDefinition, to: &StationDefinition, g: &PanelGeom) -> Option<Vec<(f64, f64)>> {
    ...
    let uses_right_side = from.kind == "sink" || to.kind == "sink" || from.id == main.id || to.id == main.id;
    ...
    if to_reserve.is_some() {
        ...
        if same_row {
            ...
            append_row_to_main(&mut points, &row.stations, from);
            push_opt(&mut points, Some(main));
            let to_row = rows.iter().find(|r| Some(&r.key) == to_reserve.as_ref()).unwrap_or(&row);
...
fn choose_corridor_row(rows: &[ReserveRow], from: &StationDefinition, to: &StationDefinition) -> ReserveRow {
...
fn append_row_to_main(points: &mut Vec<StationDefinition>, row: &[StationDefinition], from: &StationDefinition) {
...
fn append_row_from_main(points: &mut Vec<StationDefinition>, row: &[StationDefinition], to: &StationDefinition) {
...
fn point_on_polyline(points: &[(f64, f64)], phase: f64) -> (f64, f64) {
    ...
    for i in 1..points.len() {
        ...
        if target <= len || i == points.len() - 1 {
            ...
            return (points[i - 1].0 + (points[i].0 - points[i - 1].0) * local, points[i - 1].1 + (points[i].1 - points[i - 1].1) * local);
        }
        target -= len;
    }
    points[points.len() - 1]
...
fn metric_line(r: &WarehouseScenarioResult) -> String {
    let m = &r.metrics;
    [
        format!("{}/{} done", crate::des::animation::types::js_num(m.completed_jobs), crate::des::animation::types::js_num(m.jobs_created)),
...
fn cumulative_series(result: &WarehouseScenarioResult, label: &str, color: &str, value: impl Fn(&WarehouseStepTrace) -> f64) -> ChartSeries {
    ...
    ChartSeries { label: label.to_string(), color: color.to_string(), t, y }
}
