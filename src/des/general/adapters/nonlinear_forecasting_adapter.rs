...
pub fn nonlinear_forecast_schema() -> ParamSchema {
    ParamSchema::Object {
        fields: vec![
            ("trainingPeriods".to_string(), num(Some(18.0), Some(200.0), Some(true), Some(42.0))),
            ("forecastHorizon".to_string(), num(Some(1.0), Some(80.0), Some(true), Some(8.0))),
            ("mdpBudget".to_string(), num(Some(1.0), Some(10.0), Some(true), Some(6.0))),
            ("ridge".to_string(), num(Some(0.0), None, None, Some(0.03))),
            ("fineTuneIterations".to_string(), num(Some(1.0), Some(200.0), Some(true), Some(18.0))),
            ("validationShare".to_string(), num(Some(0.1), Some(0.5), None, Some(0.25))),
        ],
        required: Some(vec![]),
        description: None,
    }
    ...
    for NonlinearForecastAdapter
{
    ...
    fn summarize(
        ...
        [
            ...
            "----------------------------------------".to_string(),
            format!("  Selected variables:       {}", result.selected_variables.join(", ")),
            format!(
            ...
            format!("  POMDP final entropy:      {:.4}", result.metrics.final_belief_entropy),
            format!("  MDP states/actions:       {}/{}", result.mdp.states, result.mdp.actions),
            format!("  Equation:                 {}", result.equation.equation_text),
    ...
    fn write_csv(&self, result: &NonlinearMDPPOMDPForecastResult, csv_path: &str) {
        let mut lines =
            vec![csv_row(["kind", "t", "actual", "predicted", "lower", "upper", "split_or_belief_mode"])];
