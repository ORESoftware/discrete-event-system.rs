...
fn num(min: Option<f64>, max: Option<f64>, integer: Option<bool>, default: Option<f64>, description: Option<&str>) -> ParamSchema {
    ParamSchema::Number { min, max, integer, default, description: description.map(|s| s.to_string()) }
}
            ...
            OneOfVariant {
                ...
                schema: ParamSchema::Object {
                    fields: vec![
                        ...
                        ("Kp".to_string(), num(Some(0.0), None, None, None, Some("Proportional gain (kW/°F)"))),
                        ("Ki".to_string(), num(Some(0.0), None, None, None, Some("Integral gain (kW/°F·h)"))),
                        ("Kd".to_string(), num(Some(0.0), None, None, None, Some("Derivative gain (kW·h/°F)"))),
                    ],
                    required: Some(vec!["kind".to_string(), "Kp".to_string(), "Ki".to_string(), "Kd".to_string()]),
                    description: None,
                },
            },
            OneOfVariant {
            ...
            OneOfVariant {
                ...
                schema: ParamSchema::Object {
                    fields: vec![
                        ...
                        ("horizon_h".to_string(), num(Some(0.1), None, None, None, Some("Lookahead horizon in hours"))),
                        ("nLevels".to_string(), num(Some(2.0), Some(20.0), Some(true), None, Some("Number of discrete heater levels (2-20)"))),
                        ("comfort_penalty".to_string(), num(Some(0.0), None, None, None, None)),
                        ("cost_per_kWh".to_string(), num(Some(0.0), None, None, None, None)),
                        ("trackWeight".to_string(), num(Some(0.0), None, None, Some(1.0), Some("Soft tracking weight inside band"))),
                    ],
                    required: Some(vec![
                        "kind".to_string(),
                        "horizon_h".to_string(),
...
pub fn temp_control_schema() -> ParamSchema {
    let house_schema = ParamSchema::Object {
        fields: vec![
            ("tau".to_string(), num(Some(0.01), None, None, Some(12.0), Some("Thermal time constant (h)"))),
            ("G".to_string(), num(Some(0.0), None, None, Some(1.0), Some("Heater gain (°F per kW per hour)"))),
            ("Q_max".to_string(), num(Some(0.0), None, None, Some(5.0), Some("Max heater power (kW)"))),
            ("T_init".to_string(), num(None, None, None, Some(70.0), Some("Initial indoor temperature (°F)"))),
        ],
        required: Some(vec![]),
        description: None,
    };
    let outdoor_schema = ParamSchema::Object {
        fields: vec![
            ...
            ("amp".to_string(), num(Some(0.0), None, None, Some(15.0), None)),
            ("phase".to_string(), num(None, None, None, Some(9.0), None)),
            ("noiseStd".to_string(), num(Some(0.0), None, None, Some(1.5), None)),
        ],
        required: Some(vec![]),
        description: None,
    };
    ParamSchema::Object {
        fields: vec![
            ("T_target".to_string(), num(None, None, None, None, Some("Target indoor temperature (°F)"))),
            ("band".to_string(), num(Some(0.0), None, None, Some(2.0), Some("±band defining comfort interval (°F)"))),
            ("duration_h".to_string(), num(Some(0.0), None, None, None, Some("Simulated duration (hours)"))),
            ("dt_min".to_string(), num(Some(0.0), None, None, Some(1.0), Some("Tick length (minutes)"))),
            ("cost_per_kWh".to_string(), num(Some(0.0), None, None, Some(0.15), Some("Energy price ($/kWh)"))),
            ("comfort_penalty".to_string(), num(Some(0.0), None, None, Some(0.5), Some("Comfort violation penalty ($/(°F)²/h)"))),
            ("controller".to_string(), controller_schema()),
            ("house".to_string(), house_schema),
            ("outdoor".to_string(), outdoor_schema),
            ("sensorNoiseStd".to_string(), num(Some(0.0), None, None, Some(0.0), None)),
            ("forecastNoiseStd".to_string(), num(Some(0.0), None, None, Some(0.0), None)),
            ("forecastHorizon_h".to_string(), num(Some(0.1), None, None, Some(6.0), None)),
        ],
        required: Some(vec!["T_target".to_string(), "duration_h".to_string(), "controller".to_string()]),
...
pub fn describe_controller(c: &ControllerSpec) -> String {
    match c {
        ...
        ControllerSpec::MdpMpc { horizon_h, n_levels, track_weight, .. } => {
            format!("MDP-MPC (H={}h, levels={}, w={})", horizon_h, n_levels, track_weight.unwrap_or(1.0))
        }
    }
}
...
impl DESModelRegistration<TempControlParams, RunResult> for TempControlAdapter {
    ...
    fn summarize(&self, result: &RunResult, params: &TempControlParams) -> String {
        ...
        let max_t = result.t_in.iter().copied().fold(f64::NEG_INFINITY, f64::max);
        let lines = vec![
            "TEMPERATURE-CONTROL RUN SUMMARY".to_string(),
            "──────────────────────────────────".to_string(),
            format!("  Controller:      {}", describe_controller(&params.controller)),
            format!("  Target:          {:.2}°F ± {:.2}°F", params.t_target, params.band),
            format!(
            ...
            format!("  Comfort:         {:.2}% in band", 100.0 * result.comfort_pct),
            format!("  Violation:       {:.3} °F·h outside band", result.violation_fh),
    ...
    fn write_csv(&self, result: &RunResult, csv_path: &str) {
        let mut lines = vec!["tick,t_h,T_out,T_in,Q,energy_cum_kWh,error,in_band,violation_Fh".to_string()];
        for r in &result.trace {
            lines.push(csv_row([
                ...
                format!("{:.3}", r.error),
                if r.in_band { "1".to_string() } else { "0".to_string() },
    ...
    fn examples(&self) -> Vec<RegistrationExample<TempControlParams>> {
        vec![RegistrationExample {
            ...
            spec: DESModelSpec {
                ...
                runtime: Some(DESRuntimeConfig { seed: Some(42.0), ..Default::default() }),
                metadata: None,
            },
        }]
    }
