    ...
    FnValidator::new(name, move |s: &S| {
        ...
        vec![ValidationCheck {
            ...
            details: if passed {
                ...
                Some(format!("{mode:?}-err={err:.2e} > tol={tol}"))
            },
        }]
    })
}
    ...
    FnValidator::new(name, move |s: &S| {
        ...
        vec![ValidationCheck {
            ...
            details: if passed {
                ...
                Some(format!(
                    "value {v} outside {} interval [{low}, {high}]",
...
mod tests {
    ...
    fn monotonic_and_report() {
        let v = monotonicity_validator::<Dummy>(
            "m",
            |s| s.series.clone(),
    ...
    fn captures_panicking_validator() {
        let bad: Box<dyn Validator<Dummy>> =
            FnValidator::new("boom", |_: &Dummy| panic!("nope")).boxed();
        let s = Dummy {
            value: 0.0,
            series: vec![],
        };
