...
mod tests {
    ...
    #[test]
    fn bin_packing_force_python_requires_explicit_compatibility_value() {
        for value in [
            "1",
            "true",
            " yes ",
            ...
            assert!(
                bin_packing_reference_force_python_value(value),
                "{value:?} should enable the bin-packing compatibility bridge"
            );
        }
        ...
        for value in [
            ...
            assert!(
                !bin_packing_reference_force_python_value(value),
                "{value:?} should keep Rust bin-packing fallback active"
            );
        }
    }
