...
mod tests {
    ...
    #[test]
    fn facility_location_force_python_requires_explicit_compatibility_value() {
        for value in [
            "1",
            "true",
            " yes ",
            ...
            assert!(
                facility_location_reference_force_python_value(value),
                "{value:?} should enable the facility-location compatibility bridge"
            );
        }
        ...
        for value in [
            ...
            assert!(
                !facility_location_reference_force_python_value(value),
                "{value:?} should keep Rust facility-location fallback active"
            );
        }
    }
