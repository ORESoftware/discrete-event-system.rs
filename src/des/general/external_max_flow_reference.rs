...
mod tests {
    ...
    #[test]
    fn max_flow_force_python_requires_explicit_compatibility_value() {
        for value in [
            "1",
            "true",
            " yes ",
            ...
            assert!(
                max_flow_reference_force_python_value(value),
                "{value:?} should enable the max-flow compatibility bridge"
            );
        }
        ...
        for value in [
            ...
            assert!(
                !max_flow_reference_force_python_value(value),
                "{value:?} should keep Rust max-flow fallback active"
            );
        }
    }
