...
mod tests {
    ...
    #[test]
    fn set_cover_force_python_requires_explicit_compatibility_value() {
        for value in [
            "1",
            "true",
            " yes ",
            ...
            assert!(
                set_cover_reference_force_python_value(value),
                "{value:?} should enable the set-cover compatibility bridge"
            );
        }
        ...
        for value in [
            ...
            assert!(
                !set_cover_reference_force_python_value(value),
                "{value:?} should keep Rust set-cover fallback active"
            );
        }
    }
