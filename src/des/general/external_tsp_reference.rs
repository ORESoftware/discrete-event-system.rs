...
mod tests {
    ...
    #[test]
    fn tsp_force_python_requires_explicit_compatibility_value() {
        for value in [
            "1",
            "true",
            " yes ",
            ...
            assert!(
                tsp_reference_force_python_value(value),
                "{value:?} should enable the TSP compatibility bridge"
            );
        }
        ...
        for value in [
            ...
            assert!(
                !tsp_reference_force_python_value(value),
                "{value:?} should keep Rust TSP fallback active"
            );
        }
    }
