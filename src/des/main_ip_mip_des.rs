...
pub fn run() {
    ...
    let node_selection = match std::env::var("NODE_SELECTION").unwrap_or_else(|_| "dfs".into()).as_str() {
        "best-bound" => NodeSelection::BestBound,
        _ => NodeSelection::Dfs,
    };
    let problem = build_binary_knapsack_ip(
        vec![10.0, 40.0, 30.0, 50.0],
        vec![5.0, 4.0, 6.0, 3.0],
        10.0,
    );
    let result = solve_ipmip_with_des(
        ...
        IPMIPSolveOptions {
            ...
            allow_external_solvers: Some(std::env::var("ALLOW_EXTERNAL_SOLVERS").as_deref() == Ok("1")),
            max_nodes: Some(std::env::var("MAX_NODES").ok().and_then(|v| v.parse().ok()).unwrap_or(200)),
            max_cut_rounds: Some(std::env::var("MAX_CUT_ROUNDS").ok().and_then(|v| v.parse().ok()).unwrap_or(1)),
    ...
    println!("# root plan:        {}", result.technique_plan.root_lp_algorithm.as_str());
    ...
    println!(
        "# x*:               [{}]",
        result.x.iter().map(|v| fmt(*v, 3)).collect::<Vec<_>>().join(", ")
    );
    ...
    println!("# LP solver time:   {} ms", result.performance.total_lp_solver_ms);
    ...
    println!("# incumbent source: {}", result.incumbent_source.as_deref().unwrap_or("none"));
    println!();
    ...
    for ev in result.trace.iter().take(12) {
        ...
        let frac = if ev.fractional.is_empty() {
            ...
            format!(
                " frac={{{}}}",
                ev.fractional.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(",")
            )
        };
