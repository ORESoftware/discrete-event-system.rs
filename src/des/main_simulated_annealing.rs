...
pub fn run() {
    header("STUDY 1 — Pentagon TSP (n=5): SA finds the exact optimum");
    {
        ...
        let sa_p: Rc<dyn SAProblem<Tour>> =
            Rc::new(build_tsp_sa_problem(inst, default_tsp_sa_opts()));
    ...
    header("STUDY 2 — n=12 random TSP: SA vs Held–Karp (exact) vs GA");
    {
        ...
        println!(
            "  Held–Karp (exact)    z = {:.4}    wall = {} ms",
            exact.length, dt_exact
        );
        println!(
    ...
    {
        let t0 = Instant::now();
        let sa_p: Rc<dyn SAProblem<Tour>> = Rc::new(build_tsp_sa_problem(
        ...
        println!(
            "  SA   z = {:.4}    wall = {} ms ({} iters)",
            sa.best_cost, dt_sa, sa.iterations
        );
        println!(
            "  GA   z = {:.4}    wall = {} ms ({} generations)",
            ga.best_length, dt_ga, ga.generations
        );
        let winner = if sa.best_cost < ga.best_length {
            "SA"
        } else {
            "GA"
        };
        println!(
            "  winner: {}    margin = {:.4}",
            winner,
            (sa.best_cost - ga.best_length).abs()
        );
    }
    ...
    header("STUDY 4 — Cooling schedules on the same TSP");
    {
        ...
        println!(
            "  {:<28}{:>10}{:>8}{:>10}",
            "schedule", "best", "iters", "wall(ms)"
        );
        for (name, sched, iters) in schedules {
            let t0 = Instant::now();
            let sa_p: Rc<dyn SAProblem<Tour>> = Rc::new(build_tsp_sa_problem(
                build_random_tsp(20, 5, None),
                default_tsp_sa_opts(),
            ));
    ...
    header("STUDY 5 — 0/1 knapsack: SA heuristic vs MILP-B&B exact");
    {
        ...
        let t0 = Instant::now();
        let exact = solve_milp(
            &build_knapsack_milp(v.clone(), w.clone(), cap),
            MILPSolveOptions::default(),
        );
        let dt_exact = t0.elapsed().as_millis();
        let t1 = Instant::now();
        let knap = KnapsackInstance {
            values: v,
            weights: w,
            capacity: cap,
        };
