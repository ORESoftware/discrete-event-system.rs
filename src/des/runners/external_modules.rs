...
mod tests {
    ...
    #[test]
    fn built_in_ip_mip_external_module_is_rust_first() {
        register_built_in_external_modules().expect("register built-in modules");
        let module = get_external_module(IP_MIP_REFERENCE_ID).expect("ip/mip module");

        assert_eq!(module.source_path, "src/bin/ip_mip_reference.rs");
        ...
        assert!(
            ...
            "IP/MIP module description should advertise the Rust reference path"
        );
    }
}
