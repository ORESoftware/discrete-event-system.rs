...
//! projection of the result. A Rust external-program reference can be enabled
...
pub fn run() -> i32 {
    ...
    let (external_enabled, external_skip_detail) =
        if computer_network_external_reference_requested() {
            ...
            println!(
                "SKIP external reference module (set COMPUTER_NETWORK_REFERENCE_BACKEND=rust)"
            );
            (
                false,
