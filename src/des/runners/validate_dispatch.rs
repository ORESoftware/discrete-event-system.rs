...
fn study3(c: &mut Checker) {
    ...
    println!("    internal simplex     status={} obj={:.8}", s_internal.status, s_internal.objective);
    println!("    DES-engine simplex   status={}      obj={:.8}", s_des.status, s_des.objective);
    println!("    scipy:highs-ds       status={}  obj={:.8}", s_scipy_ds.status, s_scipy_ds.objective);
    println!("    scipy:highs-ipm      status={} obj={:.8}", s_scipy_ipm.status, s_scipy_ipm.objective);
    let objs: Vec<f64> = [&s_internal, &s_des, &s_scipy_ds, &s_scipy_ipm]
        ...
        .collect();
    if objs.len() < 2 {
        c.check("LP solvers available", false, "fewer than 2 solvers returned optimal");
        return;
    }
    let ref_obj = objs[0];
...
fn study4(c: &mut Checker) {
    ...
    let r4 = policy_mdp_vi(&problem, MdpViOpts { q_max: 4, gamma: 0.95, rollouts_per_sa: 200, seed: 42 });
    let r6 = policy_mdp_vi(&problem, MdpViOpts { q_max: 6, gamma: 0.95, rollouts_per_sa: 200, seed: 42 });
    let empty = |vi: &DispatchViResult| [vi.v[0], vi.v[1]];
    let v4 = empty(&r4);
    let v6 = empty(&r6);
    println!("    V_qMax=4(0,0,c)  = [{}]", v4.iter().map(|v| format!("{:.4}", v)).collect::<Vec<_>>().join(", "));
    println!("    V_qMax=6(0,0,c)  = [{}]", v6.iter().map(|v| format!("{:.4}", v)).collect::<Vec<_>>().join(", "));
    let max_diff = v4.iter().zip(v6.iter()).map(|(a, b)| (a - b).abs()).fold(0.0_f64, f64::max);
    c.check(
...
fn study5(c: &mut Checker) {
    ...
    let seed_base = 3300;
    let sect = evaluate_policy(&problem, || policy_sect(&problem), "sect", num_reps, num_arrivals, seed_base, warmup);
    let mcts_low = evaluate_policy(&problem, || policy_mcts(&problem, MctsOpts { iterations: 20, rollout_depth: 20 }), "mcts-20", num_reps, num_arrivals, seed_base, warmup);
    let mcts_mid = evaluate_policy(&problem, || policy_mcts(&problem, MctsOpts { iterations: 100, rollout_depth: 25 }), "mcts-100", num_reps, num_arrivals, seed_base, warmup);
    let mcts_high = evaluate_policy(&problem, || policy_mcts(&problem, MctsOpts { iterations: 300, rollout_depth: 35 }), "mcts-300", num_reps, num_arrivals, seed_base, warmup);
    println!("    SECT          mean = {:.3}", sect.mean_wait);
    ...
    let random = evaluate_policy(&problem, || policy_random(11), "random", num_reps, num_arrivals, seed_base, warmup);
    println!("    random        mean = {:.3} (sanity check)", random.mean_wait);
    c.check(
        "MCTS-300 < random (Welch-t > 3)",
        welch_t(&random.raw_waits, &mcts_high.raw_waits) > 3.0,
        &format!("t = {:.2}", welch_t(&random.raw_waits, &mcts_high.raw_waits)),
    );
    c.check(
        "MCTS-300 within 2.5× of SECT (bounded by rollout policy)",
        mcts_high.mean_wait < 2.5 * sect.mean_wait,
        &format!("MCTS = {:.3}, 2.5×SECT = {:.3}", mcts_high.mean_wait, 2.5 * sect.mean_wait),
    );
}
