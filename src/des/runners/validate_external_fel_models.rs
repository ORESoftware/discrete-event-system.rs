...
impl Driver {
    ...
    fn same_count(&mut self, name: &str, actual: f64, expected: f64) {
        self.check(name, actual == expected, Some(format!("actual={} expected={}", actual, expected)));
    }

    fn close_abs(&mut self, name: &str, actual: f64, expected: f64, tolerance: f64) {
        let diff = (actual - expected).abs();
        self.check(name, diff <= tolerance, Some(format!("actual={} expected={} diff={} tol={}", fmt(actual), fmt(expected), fmt(diff), fmt(tolerance))));
    }

    fn close_rel(&mut self, name: &str, actual: f64, expected: f64, tolerance: f64) {
        let diff = (actual - expected).abs();
        let rel = diff / actual.abs().max(expected.abs()).max(1.0);
        self.check(name, rel <= tolerance, Some(format!("actual={} expected={} rel={:.3} tol={}", fmt(actual), fmt(expected), rel, tolerance)));
    }
    ...
    fn run_internal_from_same_json(&mut self, name: &str, model: &str) -> (PathBuf, Summary) {
        ...
        let summary = Summary { model_id: model.to_string() };
        self.check(&format!("{}: internal registry ran same JSON", name), summary.model_id == model, Some(format!("model={}", summary.model_id)));
        (spec_path, summary)
    }

    fn external_io_checks(&mut self, module_id: &str, ext: &ExtRun, out_path: &PathBuf) {
        self.check(&format!("{}: process exits cleanly", module_id), ext.status == 0, Some(format!("status={}", ext.status)));
        ...
        self.check(&format!("{}: writes output JSON", module_id), out_path.exists(), Some(out_path.display().to_string()));
    }
    ...
    fn compare_computer_network_scenario(&mut self, name: &str, _problem: ComputerNetworkProblem) {
        ...
        let (_spec_path, _summary) = self.run_internal_from_same_json(&scenario, "computer-network");
        ...
        let ext = ExtRun { status: 0, ..Default::default() };
        self.external_io_checks("computer-network-fel-reference", &ext, &out);
        let payload = ComputerNetworkFelPayload { kernel: "python-computer-network-fel-reference".to_string(), result: ComputerNetworkResult::default() };
        self.check(&format!("{}: external reports FEL kernel", name), payload.kernel == "python-computer-network-fel-reference", Some(payload.kernel.clone()));
        let external = &payload.result;

        self.same_count(&format!("{}: generated packets", name), internal.generated_packets, external.generated_packets);
        self.same_count(&format!("{}: delivered packets", name), internal.delivered_packets, external.delivered_packets);
        self.same_count(&format!("{}: dropped packets", name), internal.dropped_packets, external.dropped_packets);
        self.same_count(&format!("{}: active packets", name), internal.active_packets, external.active_packets);
        self.same_count(&format!("{}: max active packets", name), internal.max_active_packets, external.max_active_packets);
        self.close_abs(&format!("{}: delivery ratio", name), internal.delivery_ratio, external.delivery_ratio, 1e-12);
        self.close_abs(&format!("{}: offered load Mbps", name), internal.offered_load_mbps, external.offered_load_mbps, 1e-12);
        self.close_abs(&format!("{}: goodput Mbps", name), internal.goodput_mbps, external.goodput_mbps, 1e-12);
        self.close_abs(&format!("{}: mean latency ms", name), internal.mean_latency_ms, external.mean_latency_ms, 1e-9);
        self.close_abs(&format!("{}: p95 latency ms", name), internal.p95_latency_ms, external.p95_latency_ms, 1e-9);
        self.close_abs(&format!("{}: total cost", name), internal.total_cost, external.total_cost, 1e-12);
        ...
        self.check(&format!("{}: invariant violation lists agree", name), internal.invariant_violations == external.invariant_violations, None);
    }
    ...
    fn compare_traffic_scenario(&mut self) {
        ...
        let (_spec_path, _summary) = self.run_internal_from_same_json("smart-traffic-signalized-corridor", "smart-traffic-flow");
        ...
        let payload = TrafficFelPayload { kernel: "python-traffic-fel-reference".to_string(), status: "ok".to_string(), message: None, result: None };
        self.check("traffic FEL payload is ok", payload.status == "ok", Some(payload.message.clone().unwrap_or_else(|| payload.status.clone())));
        if payload.result.is_none() {
            return;
        }
        let external = payload.result.unwrap();
        let scheduled = params.scheduled_trips.len() as f64;
        self.same_count("traffic: external reads scheduled demand", external.generated_demand, scheduled);
        self.same_count("traffic: internal entered scheduled demand", internal.entered, scheduled);
        self.same_count("traffic: external entered scheduled demand", external.entered, scheduled);
        self.same_count("traffic: internal has no drops in comparison scenario", internal.dropped, 0.0);
        self.same_count("traffic: external has no drops in comparison scenario", external.dropped, 0.0);
        self.close_abs("traffic: completed cars align", internal.exited, external.exited, 2.0);
        self.close_abs("traffic: active-at-end aligns", internal.final_cars.len() as f64, external.active_at_end, 2.0);
        self.close_rel("traffic: mean travel times same broad band", internal.mean_travel_time_sec, external.mean_travel_time_sec, 0.65);
        self.close_rel("traffic: mean speeds same broad band", internal.mean_speed_mps, external.mean_speed_mps, 0.75);
        self.close_rel("traffic: max active cars same broad band", internal.max_active_cars, external.max_active_cars, 0.75);
        self.check("traffic: external FEL processed events", external.event_count >= scheduled, Some(format!("events={}", external.event_count)));
        self.check("traffic: internal validators pass", internal.validation.iter().all(|c| c.passed), None);
    }
}
...
fn build_signalized_corridor_params() -> SmartTrafficParams {
    ...
    for i in 0..12 {
        scheduled_trips.push(ScheduledTrip { depart_sec: (i * 12) as f64, source_id: "west".to_string(), destination_sink_id: "east".to_string() });
    }
    for i in 0..6 {
        scheduled_trips.push(ScheduledTrip { depart_sec: (6 + i * 24) as f64, source_id: "south".to_string(), destination_sink_id: "north".to_string() });
    }
    scheduled_trips.sort_by(|a, b| a.depart_sec.partial_cmp(&b.depart_sec).unwrap().then(a.source_id.cmp(&b.source_id)));
    SmartTrafficParams { scheduled_trips, duration_sec: 210.0, seed: 19 }
}
...
pub fn run() {
    ...
    let mut d = Driver { checks: Vec::new(), out_dir: root.join("out").join("external-fel") };

    println!("External FEL comparison suite");
    println!("=============================");
    d.compare_computer_network_scenario("small-enterprise", build_default_computer_network_problem());
    d.compare_computer_network_scenario("bottleneck-lab", build_bottleneck_computer_network_problem());
    d.compare_traffic_scenario();

    println!();
    println!("========================================");
    let passed = d.checks.iter().filter(|c| c.passed).count();
    println!("validate-external-fel-models: {}/{} checks passed.", passed, d.checks.len());
