...
fn sell_execution(i: SellInput) -> SellResult {
    ...
    SellResult {
        usdc_out: sell_amount - fee_amount,
        fee_amount,
        sell_amount,
    }
}
...
fn buy_then_sell_round_trip(
    ...
    let buy = buy_execution(BuyInput {
        amount,
    ...
    let (nq1, nq2) = if is_option_one {
        (q1 + buy.shares, q2)
    } else {
        (q1, q2 + buy.shares)
    };
    let sell = sell_execution(SellInput {
        shares_out: buy.shares,
        option_one_shares: nq1,
        option_two_shares: nq2,
        b,
        fee_bps: 0.0,
        is_option_one,
    });
    RoundTripResult {
        net: sell.usdc_out - amount,
    }
}
...
fn recapitalization(q1: f64, q2: f64, current_b: f64, new_b: f64) -> RecapResult {
    ...
    RecapResult {
        new_option_one_shares: n1,
        new_option_two_shares: n2,
        new_b,
        capital_delta,
    }
}
...
impl Lmsr {
    ...
    fn buy(&mut self, amount: f64, is_yes: bool, fee_bps: f64) -> BuyResult {
        let r = buy_execution(BuyInput {
            amount,
            option_one_shares: self.q1,
            option_two_shares: self.q2,
            b: self.b,
            fee_bps,
            is_option_one: is_yes,
        });
        if is_yes {
            self.q1 += r.shares;
        } else {
    ...
    fn sell(&mut self, shares: f64, is_yes: bool, fee_bps: f64) -> SellResult {
        let r = sell_execution(SellInput {
            shares_out: shares,
            option_one_shares: self.q1,
            option_two_shares: self.q2,
            b: self.b,
...
pub fn run() {
    ...
    // A1 / A2.
    {
        ...
        for t in 0..TRIALS {
            ...
            if (p.0 + p.1 - 1.0).abs() > 1e-12 {
                c.check(
                    "A1 prices sum to 1 (sample failure)",
                    false,
                    &format!("t={}, q1={}, q2={}, sum={}", t, q1, q2, p.0 + p.1),
                );
                early = true;
                break;
            }
            if !(p.0 > 0.0 && p.0 < 1.0 && p.1 > 0.0 && p.1 < 1.0) {
                c.check(
                    "A2 prices strictly in (0, 1)",
                    false,
                    &format!("t={}, p1={}, p2={}", t, p.0, p.1),
                );
                early = true;
                break;
            }
        ...
        if !early {
            c.check(
                &format!("A1 prices sum to 1 across {} random trials", TRIALS),
                true,
                "",
            );
            c.check(
                &format!(
                    "A2 prices strictly in (0, 1) across {} random trials",
                    TRIALS
                ),
                true,
                "",
    ...
    // A5 monotonicity.
    {
        ...
        for t in 0..50 {
            ...
            if option_one_price(q1 + delta, q2, b) < option_one_price(q1, q2, b) - 1e-12 {
                c.check(
                    "A5 more shares on option-1 raises price-1",
                    false,
                    &format!("t={}", t),
                );
                early = true;
        ...
        if !early {
            c.check(
                "A5 more shares on option-1 raises price-1 across 50 trials",
                true,
                "",
            );
        }
    ...
    // A6-A9 buy invariants.
    {
        ...
        if !early {
            ...
            c.check(
                "A8 buy: averagePrice = buyAmount / shares (50 trials)",
                true,
                "",
            );
    ...
    // A12 sell monotone.
    {
        ...
        for t in 0..30 {
            ...
            let small = a.min(b_amt);
            let large = a.max(b_amt);
            let rs = sell_execution(SellInput {
                shares_out: small,
                option_one_shares: q1,
                option_two_shares: q2,
                b,
            ...
            let rl = sell_execution(SellInput {
                shares_out: large,
                option_one_shares: q1,
                option_two_shares: q2,
                b,
    ...
    // A13 round-trip no-arb.
    {
        ...
        for t in 0..100 {
            ...
            if rt.net > 1e-8 {
                c.check(
                    "A13 round-trip: sell-after-buy ≤ amount (no-arb)",
                    false,
                    &format!("t={}, net={:.2e}", t, rt.net),
                );
                early = true;
                break;
            }
    ...
    // A14 recapitalisation preserves prices.
    {
        ...
        for t in 0..30 {
            ...
            if (after.0 - before.0).abs() > 1e-10 {
                c.check(
                    "A14 recapitalisation preserves prices",
                    false,
                    &format!("t={}, |Δp|={:.2e}", t, (after.0 - before.0).abs()),
                );
                early = true;
                break;
            }
        ...
        if !early {
            c.check(
                "A14 recapitalisation preserves prices (30 trials)",
                true,
                "",
            );
        }
    ...
    // A15 slippage clamps.
    {
        ...
        for t in 0..30 {
            ...
            if !((0.0..=1.0).contains(&max) && (0.0..=1.0).contains(&min)) {
                c.check(
                    "A15 slippage clamps to [0, 1]",
                    false,
                    &format!("t={}, max={}, min={}", t, max, min),
                );
                early = true;
                break;
            }
            if !(max >= price - 1e-15 && min <= price + 1e-15) {
                c.check(
                    "A15 maxPrice ≥ price ≥ minPrice",
                    false,
                    &format!("t={}, price={}, max={}, min={}", t, price, max, min),
                );
                early = true;
                break;
            }
        ...
        if !early {
            c.check(
                "A15 slippage clamps to [0, 1] and maxPrice ≥ price ≥ minPrice (30 trials)",
                true,
                "",
            );
        }
    ...
    println!("\n=== PART B — cross-validation vs production @factmachine/math ===\n");
    let prod_trading_path = std::env::var("FACTMACHINE_TRADING_PATH").unwrap_or_else(|_| {
    ...
    println!(
        "  SKIP (production package not loadable at {})",
        prod_trading_path
    );
    println!(
        "  Set FACTMACHINE_TRADING_PATH to override; or `pnpm -C ../.. build` in the monorepo."
    );
    ...
    println!("\n=== PART C — POMDP LMSR class uses production conventions ===\n");
    {
        ...
        c.close(
            "C1 default LMSR(50, 2) gives b = 50/ln(2)",
            m.b,
            50.0 / (2.0_f64).ln(),
            1e-12,
        );
        ...
        c.close(
            "C2 LMSR(50, 2, {liquidityIsB:true}) gives b = 50",
            m_legacy.b,
            50.0,
            1e-12,
        );
        ...
        c.close(
            "C3 LMSR.binaryPrices() at q=0 returns 0.5/0.5",
            p.0,
            0.5,
            1e-12,
        );
        ...
        c.check(
            "C4 LMSR.buy(10, YES) returns positive shares",
            exec.shares > 0.0,
            &format!("shares={:.6}", exec.shares),
        );
        let p_after = m.binary_prices();
        c.check(
            "C5 P(YES) rose after buying YES",
            p_after.0 > 0.5,
            &format!("P(YES) before=0.5, after={:.6}", p_after.0),
        );
        let shares = exec.shares;
        let sell_back = m.sell(shares, true, 0.0);
        c.check(
            "C6 LMSR sell-back ≤ original amount (no-arb)",
            sell_back.usdc_out <= 10.0 + 1e-9,
            &format!("sellBack={:.6} ≤ 10", sell_back.usdc_out),
        );
    }
