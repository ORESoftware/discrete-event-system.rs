...
use crate::des::general::external_cp_sat_reference::{
    cp_sat_model_to_reference_json_string, external_cp_sat_reference_solver_specs,
    solve_cp_assignment_with_external_reference, solve_cp_sat_json_with_external_reference,
    ExternalCpSatReferenceFamily, ExternalCpSatReferenceOptions, ExternalCpSatReferenceSolver,
...
impl Driver {
    ...
    fn run_cp_sat_rust_json(&self, args: &[&str], stdin_json: &str) -> serde_json::Value {
        let mut enumerate_solutions = None;
        let mut assumption_core = false;
        let mut idx = 0;
        while idx < args.len() {
            match args[idx] {
                "--enumerate-solutions" => {
                    let Some(limit) = args.get(idx + 1) else {
                        panic!("--enumerate-solutions requires a limit");
                    };
                    enumerate_solutions = Some(
                        limit
                            .parse::<usize>()
                            .expect("parse CP-SAT enumerate-solution limit"),
                    );
                    idx += 2;
                }
                "--assumption-core" => {
        ...
        let model = serde_json::from_str(stdin_json).expect("parse CP-SAT JSON reference model");
    ...
    fn check_cp_reference_optimal(
        ...
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &reference_json.to_string());
    ...
    fn validate_external_solver_clis(&mut self) {
        ...
        for solver in ExternalLinearCliSolver::open_source_lp().iter().copied() {
            ...
            if mps_reference.status == ExternalLinearCliStatus::Unavailable
                && mps_reference.message.contains("not found")
            {
                println!("  SKIP  LP {solver_name}:rust-cli MPS executable not found");
                continue;
            }
            self.check(
        ...
        for solver in ExternalLinearCliSolver::open_source_mip().iter().copied() {
            ...
            if mps_reference.status == ExternalLinearCliStatus::Unavailable
                && mps_reference.message.contains("not found")
            {
                println!("  SKIP  IP/MIP {solver_name}:rust-cli MPS executable not found");
                continue;
            }
            self.check(
    ...
    fn validate_external_optimization_ecosystem_adapters(&mut self) {
        ...
        let linear_report = run_external_optimization_comparison(
            ...
            &[
                ...
                ecosystem_invocation("lp-modeler-reference", ExternalOptimizationTool::LpModeler),
                ecosystem_invocation(
        ...
        self.check(
            "External optimization ecosystem planning reference bridge",
            planning_report.agreement
                && planning_report.reference_objective == Some(5.0)
                && planning_report.reference_solution.as_deref() == Some(&[0.0, 0.0, 1.0]),
            format!(
                "agreement={} objective={:?} solution={:?}",
                planning_report.agreement,
                planning_report.reference_objective,
                planning_report.reference_solution
            ),
        );
        ...
        let multiobjective_report = run_external_optimization_comparison(
            ...
            &[
                ...
                ecosystem_invocation("ecj-reference", ExternalOptimizationTool::Ecj),
            ],
            1e-9,
            1e-9,
        );
        ...
        let reference_bridge_count: usize = reference_reports
            .iter()
            .map(|report| report.runs.len())
            .sum();
        let all_requested_tools_have_reference_bridge =
            external_optimization_tools().iter().copied().all(|tool| {
                reference_reports
                    .iter()
                    .any(|report| report.runs.iter().any(|run| run.run.tool == tool))
            });
        self.check(
    ...
    fn validate_external_validation_tool_adapters(&mut self) {
        ...
        self.check(
            ...
            specs.iter().any(|spec| spec.id == "minizinc")
                ...
                && specs.iter().any(|spec| spec.id == "csv-validator")
        ...
        for tool in [
            ...
            self.check(
                ...
                registered_tool_run["status"].as_str() == Some("ok")
                    && registered_tool_run["verdict"].as_str() == Some("sat")
                    && registered_tool_run["stdout"]
                        .as_str()
                        .is_some_and(|stdout| stdout.contains("x =")),
                format!(
                    "status={} verdict={} validator={}",
                    registered_tool_run["status"].as_str().unwrap_or(""),
                    registered_tool_run["verdict"].as_str().unwrap_or(""),
        ...
        self.check(
            ...
            yices_smt_bridge_run["status"].as_str() == Some("ok")
                && yices_smt_bridge_run["verdict"].as_str() == Some("sat"),
            format!(
                "status={} verdict={} validator={}",
                yices_smt_bridge_run["status"].as_str().unwrap_or(""),
                yices_smt_bridge_run["verdict"].as_str().unwrap_or(""),
                yices_smt_bridge_run["validator"].as_str().unwrap_or("")
            ),
        );
        ...
        for tool in [
            ...
            self.check(
                ...
                registered_smt_run["status"].as_str() == Some("ok")
                    && registered_smt_run["verdict"].as_str() == Some("sat"),
                format!(
                    "status={} verdict={} validator={}",
                    registered_smt_run["status"].as_str().unwrap_or(""),
                    registered_smt_run["verdict"].as_str().unwrap_or(""),
                    registered_smt_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        for tool in [
            ...
            self.check(
                ...
                registered_smt_unsat_run["status"].as_str() == Some("ok")
                    && registered_smt_unsat_run["verdict"].as_str() == Some("unsat"),
                format!(
                    "status={} verdict={} validator={}",
                    registered_smt_unsat_run["status"].as_str().unwrap_or(""),
                    registered_smt_unsat_run["verdict"].as_str().unwrap_or(""),
                    registered_smt_unsat_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        for tool in ["tlc", "apalache"] {
            ...
            self.check(
                ...
                registered_tla_run["status"].as_str() == Some("ok")
                    && registered_tla_run["verdict"].as_str() == Some("valid")
                    && registered_tla_run["validator"].as_str() == Some("builtin:tla-structural"),
                format!(
                    "status={} verdict={} validator={}",
                    registered_tla_run["status"].as_str().unwrap_or(""),
                    registered_tla_run["verdict"].as_str().unwrap_or(""),
                    registered_tla_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        let kodkod_bridge_payload = serde_json::json!({
            "kind": "kodkod-validation",
            "model": "module smoke\nsig Node { next: lone Node }\npred someNode { some Node }\n",
            "commands": ["run someNode for 3"],
        });
        ...
        self.check(
            ...
            kodkod_bridge_run["status"].as_str() == Some("ok")
                && kodkod_bridge_run["verdict"].as_str() == Some("valid")
                && kodkod_bridge_run["validator"].as_str() == Some("builtin:alloy-structural"),
            format!(
                "status={} verdict={} validator={}",
                kodkod_bridge_run["status"].as_str().unwrap_or(""),
                kodkod_bridge_run["verdict"].as_str().unwrap_or(""),
                kodkod_bridge_run["validator"].as_str().unwrap_or("")
            ),
        );
        ...
        for (tool, payload) in [
            (
                ...
                serde_json::json!({
                    "kind": "program-verifier-validation",
                    "language": "frama-c",
                    "source": "/*@ requires x >= 0; ensures \\result >= x; */\nint inc(int x) { return x + 1; }\n",
                }),
            ),
            (
                "why3",
            ...
            (
                ...
                serde_json::json!({
                    "kind": "program-verifier-validation",
                    "language": "jbmc",
                    "source": "class Smoke { void test() { int x = 1; assert(x == 1); } }\n",
                }),
            ),
            (
                "coq",
                serde_json::json!({
                    "kind": "program-verifier-validation",
                    "language": "coq",
                    "source": "Theorem smoke : True.\nProof. exact I. Qed.\n",
                    "contracts": ["theorem smoke"],
                }),
            ),
            (
                "isabelle",
                serde_json::json!({
                    "kind": "program-verifier-validation",
                    "language": "isabelle",
                    "source": "theory Smoke imports Main begin\nlemma smoke: True by simp\nend\n",
                }),
            ),
            (
                "lean",
                serde_json::json!({
                    "kind": "program-verifier-validation",
                    "language": "lean",
                    "source": "theorem smoke : True := True.intro\n",
                }),
            ),
            (
                "pvs",
            ...
            self.check(
                ...
                registered_program_verifier_run["status"].as_str() == Some("ok")
                    && registered_program_verifier_run["verdict"].as_str() == Some("valid")
                    && registered_program_verifier_run["validator"].as_str()
                        == Some("builtin:program-verifier-structural"),
                format!(
                    "status={} verdict={} validator={}",
                    registered_program_verifier_run["status"]
                        .as_str()
                        .unwrap_or(""),
                    registered_program_verifier_run["verdict"]
                        .as_str()
                        .unwrap_or(""),
                    registered_program_verifier_run["validator"]
                        .as_str()
                        .unwrap_or("")
                ),
            );
        ...
        for (tool, payload) in [
            (
                ...
                serde_json::json!({
                    "kind": "security-protocol-validation",
                    "model": "free c: channel.\nfree secret: bitstring [private].\nprocess out(c, secret)\n",
                    "queries": ["query attacker(secret)."],
                }),
            ),
            (
                "cryptoverif",
            ...
            (
                ...
                serde_json::json!({
                    "kind": "security-protocol-validation",
                    "model": "process Smoke = out(c, secret)\nquery secrecy secret\n",
                }),
            ),
        ] {
            ...
            self.check(
                ...
                registered_security_run["status"].as_str() == Some("ok")
                    && registered_security_run["verdict"].as_str() == Some("valid")
                    && registered_security_run["validator"].as_str()
                        == Some("builtin:security-protocol-structural"),
                format!(
                    "status={} verdict={} validator={}",
                    registered_security_run["status"].as_str().unwrap_or(""),
                    registered_security_run["verdict"].as_str().unwrap_or(""),
                    registered_security_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        for (tool, family, format, path) in [
            ("miplib", "mip", "mps", "sample.mps"),
            ("qplib", "qp", "qplib", "sample.qplib"),
            ("minlplib", "minlp", "nl", "sample.nl"),
            ("netlib-lp", "lp", "json", "sample.json"),
            ("csplib", "cp", "json", "sample.json"),
            ("or-library", "or", "json", "sample.json"),
            ("tsplib", "routing", "json", "sample.json"),
            ("vrplib", "routing", "json", "sample.json"),
            ("minizinc-challenge", "cp", "dzn", "sample.dzn"),
        ] {
            let registered_benchmark_payload = serde_json::json!({
                "suite": tool,
                "entries": [
            ...
            self.check(
                ...
                registered_benchmark_run["status"].as_str() == Some("ok")
                    && registered_benchmark_run["verdict"].as_str() == Some("valid")
                    && registered_benchmark_run["checks"]
                        .as_array()
                        .is_some_and(|checks| !checks.is_empty()),
                format!(
                    "status={} verdict={} validator={}",
                    registered_benchmark_run["status"].as_str().unwrap_or(""),
                    registered_benchmark_run["verdict"].as_str().unwrap_or(""),
                    registered_benchmark_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        let nonlinear_run = solve_nonlinear_validation_json_with_external_reference(
        ...
        self.check(
            ...
            nonlinear_run.status == ExternalNonlinearValidationReferenceStatus::Optimal
                && nonlinear_run
                    .objective
                    .is_some_and(|objective| objective.abs() <= 1e-6)
                && nonlinear_run.x.len() == 2,
            format!(
                "status={} objective={} solver={}",
                nonlinear_run.status.as_str(),
                nonlinear_run.objective.unwrap_or(f64::NAN),
                nonlinear_run.solver
            ),
        );
        ...
        let infeasible_nonlinear_run = solve_nonlinear_validation_json_with_external_reference(
            infeasible_nonlinear_payload,
            &ExternalNonlinearValidationReferenceOptions {
        ...
        self.check(
            ...
            infeasible_nonlinear_run.status
                == ExternalNonlinearValidationReferenceStatus::Infeasible
                && infeasible_nonlinear_run
                    .message
                    .contains("constraint violation"),
            format!(
                "status={} solver={} message={}",
                infeasible_nonlinear_run.status.as_str(),
                infeasible_nonlinear_run.solver,
        ...
        self.check(
            ...
            csv_validator_run["status"].as_str() == Some("ok")
                && csv_validator_run["verdict"].as_str() == Some("valid")
                && csv_validator_run["validator"].as_str() == Some("builtin:table-schema-subset"),
            format!(
                "status={} verdict={} validator={}",
                csv_validator_run["status"].as_str().unwrap_or(""),
                csv_validator_run["verdict"].as_str().unwrap_or(""),
        ...
        let xml_schema_payload = serde_json::json!({
            "kind": "xsd-validation",
            "xml": "<run><objective>3.5</objective></run>",
        ...
        self.check(
            ...
            xml_schema_run["status"].as_str() == Some("ok")
                && xml_schema_run["verdict"].as_str() == Some("valid")
                && xml_schema_run["validator"].as_str() == Some("builtin:xml-schema-structural"),
            format!(
                "status={} verdict={} validator={}",
                xml_schema_run["status"].as_str().unwrap_or(""),
                xml_schema_run["verdict"].as_str().unwrap_or(""),
        ...
        for tool in ["jing", "saxon"] {
            ...
            self.check(
                ...
                format!(
                    ...
                    xml_rule_run["verdict"].as_str().unwrap_or(""),
                    xml_rule_run["validator"].as_str().unwrap_or("")
                ),
            );
        }
        ...
        self.check(
            ...
            protoc_run["status"].as_str() == Some("ok")
                && protoc_run["verdict"].as_str() == Some("valid")
                && protoc_run["validator"].as_str() == Some("builtin:protobuf-conformance-subset"),
            format!(
                "status={} verdict={} validator={}",
                protoc_run["status"].as_str().unwrap_or(""),
                protoc_run["verdict"].as_str().unwrap_or(""),
                protoc_run["validator"].as_str().unwrap_or("")
            ),
        );
        ...
        self.check(
            ...
            apache_avro_run["status"].as_str() == Some("ok")
                && apache_avro_run["verdict"].as_str() == Some("valid")
                && apache_avro_run["validator"].as_str() == Some("builtin:avro-schema-subset"),
            format!(
                "status={} verdict={} validator={}",
                apache_avro_run["status"].as_str().unwrap_or(""),
                apache_avro_run["verdict"].as_str().unwrap_or(""),
                apache_avro_run["validator"].as_str().unwrap_or("")
            ),
        );
    ...
    fn validate_math_program_facade(&mut self) {
        ...
        let false_indicator_highs_opts = ExternalMathProgramOptions {
            presolve: Some(ExternalLinearCliPresolve::Off),
            ..external_opts.clone()
        };
        match cross_check_math_program_with_external(
            ...
            Ok(report) => self.check(
                ...
                report.within_tolerance
                    ...
                    && report.objective_abs_diff.is_some_and(|diff| diff <= 1e-7)
                    && report.max_x_abs_diff.is_some_and(|diff| diff <= 1e-7)
                    && report.external.control_feedback.as_ref().is_some_and(|feedback| {
                ...
                format!(
                    "internal={:?} external={:?} obj_diff={:?} x_diff={:?} objective={} x={:?} violations=({:?},{:?}) control_feedback={:?}",
                    report.internal.status,
                    report.external.status,
                    report.objective_abs_diff,
                    report.max_x_abs_diff,
                    ...
                    report.internal_max_violation,
                    report.external_max_violation,
                    report.external.control_feedback
                ),
            ),
            Err(err) => self.check(
                "MathProgram MIP facade false-active indicator HiGHS cross-check",
        ...
        let logic_xor = general_mip
            ...
            .expect("logic xor");
        let signed_logic_and = general_mip
            .add_binary_var("signed-logic-and", 1.0)
            .expect("signed logic and");
        let signed_logic_or = general_mip
            .add_binary_var("signed-logic-or", 1.0)
            .expect("signed logic or");
        let signed_logic_xor = general_mip
            .add_binary_var("signed-logic-xor", 1.0)
            .expect("signed logic xor");
        ...
        let reified_equality = general_mip
            .add_binary_var("reified-logic-equality", 5.0)
            .expect("reified logic equality");
        ...
        general_mip
            ...
            .expect("signed logical and");
        general_mip
            .add_boolean_or(
                "signed-logical-or",
                signed_logic_or,
                vec![
                    MathProgram::not_lit(logic_a),
                    MathProgram::bool_lit(logic_b),
                ],
            )
            .expect("signed logical or");
        general_mip
            .add_boolean_xor(
                "signed-logical-xor",
                signed_logic_xor,
        ...
        general_mip
            .add_reified_le_constraint(
                ...
                vec![(logic_a, 1.0), (logic_c, 1.0)],
                2.0,
            )
            .expect("reified logic threshold");
        general_mip
            .add_reified_eq_constraint(
                "reified-logic-equality",
                MathProgram::bool_lit(reified_equality),
                vec![(logic_a, 1.0), (logic_c, 1.0)],
                2.0,
            )
            .expect("reified logic equality");
        general_mip
            .add_linear_domain(
                "linear-expression-domain",
                vec![(linear_domain_value, 1.0)],
                vec![(1, 2), (4, 4)],
            )
        ...
        let l0_x = general_mip
            .add_integer_var("l0-x", 10.0, Some(-1.0), Some(1.0))
            .expect("l0 x");
        let l0_y = general_mip
            .add_integer_var("l0-y", -5.0, Some(0.0), Some(1.0))
            .expect("l0 y");
        let l0_z = general_mip
            .add_integer_var("l0-z", 2.0, Some(0.0), Some(2.0))
            .expect("l0 z");
        let l0_norm = general_mip
            .add_integer_var("l0-norm", 4.0, Some(0.0), Some(3.0))
            .expect("l0 norm");
        general_mip
            .add_l0_norm("l0-norm-constraint", l0_norm, vec![l0_x, l0_y, l0_z])
            .expect("l0 norm constraint");
            ...
            Ok(report) => {
                ...
                self.check(
                    ...
                    report.within_tolerance
                        ...
                        && (report.internal.x[signed_logic_and] - 1.0).abs() <= 1e-7
                        ...
                        && (report.internal.x[l0_x] - 1.0).abs() <= 1e-7
                        && report.internal.x[l0_y].abs() <= 1e-7
                        && (report.internal.x[l0_z] - 2.0).abs() <= 1e-7
                        && (report.internal.x[l0_norm] - 2.0).abs() <= 1e-7
        ...
        match export_math_program_cplex_lp(&general_mip) {
            Ok(export) => {
                ...
                let passed = export.is_mip
                    ...
                    && has_generated("signed-logical-and")
                    ...
                    && has_generated("l0-norm-constraint")
        ...
        let count_x0 = table_mip
            .add_integer_var("count-x0", 100.0, Some(0.0), Some(2.0))
            .expect("count x0");
        let count_x1 = table_mip
            .add_integer_var("count-x1", 10.0, Some(0.0), Some(2.0))
            .expect("count x1");
        let count_x2 = table_mip
            .add_integer_var("count-x2", 1.0, Some(0.0), Some(2.0))
            .expect("count x2");
        let count_x3 = table_mip
            .add_integer_var("count-x3", 0.0, Some(0.0), Some(2.0))
            .expect("count x3");
        table_mip
            .add_global_cardinality(
                "global-counts",
                vec![count_x0, count_x1, count_x2, count_x3],
                vec![
            ...
            .expect("global cardinality");
        let count_ones = table_mip
            .add_integer_var("count-ones", 30.0, Some(0.0), Some(4.0))
            .expect("count ones");
        table_mip
            .add_value_count(
                "count-ones-value",
        ...
        let literal_a = table_mip
            ...
            .expect("literal a");
        let literal_b = table_mip
            .add_binary_var("literal-b", -3.0)
            .expect("literal b");
        let literal_c = table_mip
            .add_binary_var("literal-c", -4.0)
            .expect("literal c");
        let literal_d = table_mip
            .add_binary_var("literal-d", 4.0)
            .expect("literal d");
        table_mip
            .add_literal_exactly_k(
                "signed-literal-exactly-two",
                vec![
                    MathProgram::bool_lit(literal_a),
                    MathProgram::not_lit(literal_b),
                    MathProgram::bool_lit(literal_c),
                    MathProgram::not_lit(literal_d),
                ],
                2,
            )
            .expect("signed literal cardinality");
            ...
            Ok(report) => {
                ...
                self.check(
                    ...
                    report.within_tolerance
                        ...
                        && (report.internal.x[count_x0] - 2.0).abs() <= 1e-7
                        && (report.internal.x[count_x1] - 1.0).abs() <= 1e-7
                        ...
                        && (report.internal.x[literal_d] - 1.0).abs() <= 1e-7
    ...
    fn validate_qp(&mut self) {
        ...
        let reference = solve_qp_with_external_reference(
            &qp,
            &ExternalQuadraticReferenceOptions {
                solver: ExternalQuadraticReferenceSolver::RustInternal,
                ..Default::default()
        ...
        self.check(
            ...
            internal.status == QPStatus::Optimal
                && reference.status == ExternalQuadraticReferenceStatus::Optimal,
            format!(
                "internal={} external={} solver={}",
                internal.status.as_str(),
                reference.status.as_str(),
        ...
        let bound_reference = solve_qp_with_external_reference(
            &bound_qp,
            &ExternalQuadraticReferenceOptions {
                solver: ExternalQuadraticReferenceSolver::RustInternal,
                ..Default::default()
        ...
        self.check(
            ...
            bound_internal.status == QPStatus::Optimal
                && bound_reference.status == ExternalQuadraticReferenceStatus::Optimal,
            format!(
                "internal={} external={} solver={}",
                bound_internal.status.as_str(),
                bound_reference.status.as_str(),
        ...
        let miqp_reference = solve_miqp_with_external_reference(
            &miqp,
            &ExternalQuadraticReferenceOptions {
                solver: ExternalQuadraticReferenceSolver::RustInternal,
                ..Default::default()
        ...
        self.check(
            ...
            miqp_internal.status == QPStatus::Optimal
                && miqp_reference.status == ExternalQuadraticReferenceStatus::Optimal,
            format!(
                "internal={} external={} solver={}",
                miqp_internal.status.as_str(),
                miqp_reference.status.as_str(),
        ...
        let socp_reference = solve_socp_with_external_reference(
            &socp,
            &ExternalQuadraticReferenceOptions {
                solver: ExternalQuadraticReferenceSolver::RustInternal,
                ..Default::default()
        ...
        self.check(
            ...
            socp_internal.status == SocpStatus::Optimal
                && socp_reference.status == ExternalQuadraticReferenceStatus::Optimal,
            format!(
                "internal={} external={} solver={}",
                socp_internal.status.as_str(),
                socp_reference.status.as_str(),
        ...
        for solver in [
            ...
            let optional_socp_reference = solve_socp_with_external_reference(
                &socp,
                &ExternalQuadraticReferenceOptions {
                    solver,
                    ..Default::default()
            ...
            self.check(
                format!("SOCP registered {} bridge status optimal", solver.as_arg()),
                optional_socp_reference.status == ExternalQuadraticReferenceStatus::Optimal,
                format!(
                    "external={} solver={}",
                    optional_socp_reference.status.as_str(),
                    optional_socp_reference.solver
                ),
            );
            let objective_check = format!("SOCP registered {} objective", solver.as_arg());
            self.close(
                &objective_check,
                socp_internal.objective,
            ...
            let x_check = format!("SOCP registered {} x", solver.as_arg());
        ...
        let qcp_reference = solve_qcp_with_external_reference(
            &qcp,
            &ExternalQuadraticReferenceOptions {
                solver: ExternalQuadraticReferenceSolver::RustInternal,
                ..Default::default()
        ...
        self.check(
            ...
            qcp_internal.status == QcpStatus::Optimal
                && qcp_reference.status == ExternalQuadraticReferenceStatus::Optimal,
            format!(
                "internal={} external={} solver={}",
                qcp_internal.status.as_str(),
                qcp_reference.status.as_str(),
        ...
        for solver in [
            ...
            let optional_qcp_reference = solve_qcp_with_external_reference(
                &qcp,
                &ExternalQuadraticReferenceOptions {
                    solver,
                    ..Default::default()
            ...
            self.check(
                format!("QCP registered {} bridge status optimal", solver.as_arg()),
                optional_qcp_reference.status == ExternalQuadraticReferenceStatus::Optimal,
                format!(
                    "external={} solver={}",
                    optional_qcp_reference.status.as_str(),
                    optional_qcp_reference.solver
                ),
            );
            let objective_check = format!("QCP registered {} objective", solver.as_arg());
            self.close(
                &objective_check,
                qcp_internal.objective,
    ...
    fn validate_cp_sat(&mut self) {
        ...
        let internal = solve_cp_model(&model, CpSolveOptions::default());
        let model_json = cp_sat_model_to_reference_json_string(&model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &model_json);
        ...
        let variable_element_json = cp_sat_model_to_reference_json_string(&variable_element_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &variable_element_json);
        ...
        let enumeration_json = cp_sat_model_to_reference_json_string(&enumeration_model);
        let value = self.run_cp_sat_rust_json(
        ...
        .to_string();
        let value =
            self.run_cp_sat_rust_json(&["--solver", "auto", "--assumption-core"], &assumption_json);
        ...
        let optional_interval_json =
            cp_sat_model_to_reference_json_string(&optional_interval_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &optional_interval_json);
        ...
        let variable_interval_json =
            cp_sat_model_to_reference_json_string(&variable_interval_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &variable_interval_json);
        ...
        let alternative_json = cp_sat_model_to_reference_json_string(&alternative_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &alternative_json);
        ...
        let variable_no_overlap_2d_json =
            cp_sat_model_to_reference_json_string(&variable_no_overlap_2d_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &variable_no_overlap_2d_json);
        ...
        let variable_cumulative_json =
            cp_sat_model_to_reference_json_string(&variable_cumulative_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &variable_cumulative_json);
        ...
        .to_string();
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &hinted_json);
        ...
        .to_string();
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &strategy_json);
        ...
        for (label, domain_strategy, domain_strategy_json, expected_assignment) in [
            ...
            .to_string();
            let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &domain_strategy_json);
        ...
        let enforced_bool_json = cp_sat_model_to_reference_json_string(&enforced_bool_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &enforced_bool_json);
        ...
        let at_least_one_json = cp_sat_model_to_reference_json_string(&at_least_one_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &at_least_one_json);
        ...
        let enforced_linear_domain_json =
            cp_sat_model_to_reference_json_string(&enforced_linear_domain_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &enforced_linear_domain_json);
        ...
        let enforced_cardinality_json =
            cp_sat_model_to_reference_json_string(&enforced_cardinality_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &enforced_cardinality_json);
        ...
        let enforced_table_json = cp_sat_model_to_reference_json_string(&enforced_table_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &enforced_table_json);
        ...
        let feasible_json = cp_sat_model_to_reference_json_string(&feasible_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &feasible_json);
        ...
        let infeasible_json = cp_sat_model_to_reference_json_string(&infeasible_model);
        let value = self.run_cp_sat_rust_json(&["--solver", "auto"], &infeasible_json);
