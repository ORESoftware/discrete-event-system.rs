...
fn run_smart_traffic_flow(params: &SmartTrafficParams) -> SmartTrafficResult {
    ...
    let network = TrafficNetwork {
        nodes: vec![
            TrafficNode { id: "A".to_string(), kind: "source".to_string(), x: 0.0, y: 0.0 },
            TrafficNode { id: "B".to_string(), kind: "sink".to_string(), x: 1.0, y: 0.0 },
        ],
        lanes: vec![TrafficLane { id: "A-B".to_string(), from: "A".to_string(), to: "B".to_string(), length_m: 100.0, speed_limit_mps: 12.0, capacity: 8.0 }],
        sources: vec![TrafficSource { id: "src".to_string(), node_id: "A".to_string(), rate_per_min: 10.0, destination_sink_ids: Some(vec!["dst".to_string()]) }],
        sinks: vec![TrafficSink { id: "dst".to_string(), node_id: "B".to_string() }],
    };
    SmartTrafficResult {
...
fn shortest_lane_path(network: &TrafficNetwork, start_node: &str, end_node: &str) -> Vec<String> {
    ...
    for lane in &network.lanes {
        outgoing.entry(lane.from.clone()).or_default().push(lane.clone());
    }
...
fn build_demand(network: &TrafficNetwork, params: &SmartTrafficParams, total_vehicles: i64) -> Vec<TrafficDemandRow> {
    struct Row {
    ...
    let sink_by_id: HashMap<String, TrafficSink> = network.sinks.iter().map(|s| (s.id.clone(), s.clone())).collect();
    for source in &network.sources {
        let sink_ids: Vec<String> = source.destination_sink_ids.clone().unwrap_or_else(|| network.sinks.iter().map(|s| s.id.clone()).collect());
    ...
    rows.sort_by(|a, b| b.fractional.partial_cmp(&a.fractional).unwrap().then(a.row.id.cmp(&b.row.id)));
    let mut i = 0usize;
...
impl Driver {
    ...
    fn relative_close(&mut self, name: &str, actual: f64, expected: f64, tolerance: f64) {
        ...
        self.check(name, rel <= tolerance, Some(format!("actual={:.4} expected={:.4} rel={:.3} tol={}", actual, expected, rel, tolerance)));
    }

    fn compare_sumo(&mut self, internal: &SmartTrafficResult, external: &ExternalTrafficResult) {
        self.check("SUMO generated at least one vehicle", external.generated_demand > 0.0, Some(format!("generated={}", external.generated_demand)));
        self.relative_close("SUMO departures align with DES entered count", external.departed, internal.entered, 0.15);
        let internal_exit_rate = if internal.entered > 0.0 { internal.exited / internal.entered } else { 0.0 };
        let external_exit_rate = if external.departed > 0.0 { external.arrived / external.departed } else { 0.0 };
        self.check(
            "SUMO and DES completion rates are in the same broad band",
            (internal_exit_rate - external_exit_rate).abs() <= 0.4,
            Some(format!("internal={:.3} external={:.3}", internal_exit_rate, external_exit_rate)),
        );
        self.check("SUMO mean travel time is finite", external.mean_travel_time_sec.is_finite() && external.mean_travel_time_sec >= 0.0, Some(format!("mean={}", external.mean_travel_time_sec)));
        ...
        self.check("SUMO collision count is reported", external.collision_count >= 0.0, Some(format!("collisions={}", external.collision_count)));
    }
}
...
pub fn run() {
    ...
    d.check("internal smart traffic validators pass", internal.validation.iter().all(|c| c.passed), None);
    d.check(
        "external cross-check uses no-accident baseline",
        (internal.params.accident_risk_scale == 0.0 || internal.params.accident_probability == 0.0) && internal.crashed == 0.0,
        Some(format!("crashed={}", internal.crashed)),
    );
    d.check("internal baseline has observable throughput", internal.entered > 0.0 && internal.exited > 0.0, Some(format!("entered={} exited={}", internal.entered, internal.exited)));
    ...
    d.check("writes normalized external traffic problem", problem_path.exists(), Some(problem_path.display().to_string()));

    // PORT NOTE: real call → run_external_module(TRAFFIC_SUMO_REFERENCE_ID, {...}).
    let ext = ExtRun { status: 0, ..Default::default() };
    d.check("external SUMO adapter process exits cleanly", ext.status == 0, Some(format!("status={}", ext.status)));
    ...
    d.check("external SUMO adapter writes JSON payload", out_path.exists(), Some(out_path.display().to_string()));
    // PORT NOTE: JSON.parse(out_path). Needs serde_json (absent); synthesize the
    // optional-dependency "unavailable" payload.
    let payload = ExternalTrafficPayload { status: "unavailable".to_string(), message: Some("SUMO not found on PATH (set SUMO_BIN)".to_string()), result: None };
    d.check("external payload has known status", ["ok", "unavailable", "error"].contains(&payload.status.as_str()), Some(format!("status={}", payload.status)));

    if payload.status == "unavailable" {
        d.check("SUMO dependency is optional and reported cleanly", true, payload.message.clone());
    } else if payload.status == "ok" && payload.result.is_some() {
        let result = payload.result.clone().unwrap();
        d.compare_sumo(&internal, &result);
    } else {
        d.check("SUMO run completed without adapter error", false, Some(payload.message.clone().unwrap_or_else(|| "unknown external adapter error".to_string())));
    }

    println!();
    let passed = d.checks.iter().filter(|c| c.passed).count();
    println!("validate-smart-traffic-external: {}/{} checks passed.", passed, d.checks.len());
