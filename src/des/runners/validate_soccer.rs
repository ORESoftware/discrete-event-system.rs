...
fn evaluate_schedule(_p: &SoccerProblem, _s: &Schedule) -> ScheduleEval {
    ScheduleEval { affinity_sum: 0.0, fairness_ok: true, fairness_violations: vec![] }
}
...
fn run_many_matches(_p: &SoccerProblem, _s: &Schedule, _name: &str, num_matches: usize, _seed: u64) -> ManyMatchesResult {
    ManyMatchesResult { mean_goal_diff: 0.0, sd_goal_diff: 0.0, raw_goal_diffs: vec![0.0; num_matches], fairness_ok: true }
}
...
fn stub_lp() -> LpSolution {
    LpSolution { status: "optimal".to_string(), objective: 0.0 }
}
...
pub fn run() {
    ...
    println!("\nStudy 1 — LP relaxation upper bound = MDP-VI exact optimum");
    {
        ...
        println!("    LP-rounded affinity  = {:.6}", evaluate_schedule(&problem, &lp.schedule).affinity_sum);
        ...
        c.check("MDP-VI affinity = MDP optimum (consistent)", (mdp_eval.affinity_sum - mdp.optimal_value).abs() < 1e-9, "");
        c.check("MDP-VI is structurally valid", validate_schedule_structure(&problem, &mdp.schedule).is_none(), "");
        c.check("MDP-VI satisfies fairness constraint", mdp_eval.fairness_ok, &format!("{} violations", mdp_eval.fairness_violations.len()));
    }
    ...
    println!("\nStudy 2 — State augmentation: memoryless MDP violates, augmented MDP enforces");
    {
        ...
        println!("    state = (t,)              affinity = {:.4}, fairness = {}", mem_eval.affinity_sum, mem_eval.fairness_ok);
        println!("    state = (t, prev_bench)   affinity = {:.4}, fairness = {}", aug_eval.affinity_sum, aug_eval.fairness_ok);
        println!("    cost of fairness          = {:.4}", mem_eval.affinity_sum - aug_eval.affinity_sum);
        c.check(
        ...
        c.check("memoryless MDP VIOLATES fairness (no history in state ⇒ constraint inexpressible)", !mem_eval.fairness_ok, &format!("{} violations", mem_eval.fairness_violations.len()));
        c.check("augmented MDP RESPECTS fairness (history in state ⇒ constraint enforced)", aug_eval.fairness_ok, &format!("0 violations expected, got {}", aug_eval.fairness_violations.len()));
        c.check("augmented MDP optimum within the LP upper bound", aug_eval.affinity_sum <= policy_lp_relaxed(&problem).lp_value + 1e-9, "");
    }
    ...
    println!("\nStudy 3 — Cross-solver consistency on the soccer LP");
    {
        ...
        println!("    internal simplex     status={} obj={:.8}", s_int.status, s_int.objective);
        println!("    DES-engine simplex   status={} obj={:.8}", s_des.status, s_des.objective);
        println!("    scipy:highs-ds       status={} obj={:.8}", s_ds.status, s_ds.objective);
        println!("    scipy:highs-ipm      status={} obj={:.8}", s_ipm.status, s_ipm.objective);
        let objs: Vec<f64> = [&s_int, &s_des, &s_ds, &s_ipm].iter().filter(|s| s.status == "optimal").map(|s| s.objective).collect();
        if objs.len() < 2 {
            c.check("At least two LP solvers available", false, "cannot cross-validate");
        } else {
            let ref_obj = objs[0];
            let max_diff = objs.iter().map(|o| (o - ref_obj).abs()).fold(0.0_f64, f64::max);
            c.check("all available solvers agree on the LP objective to 1e-5", max_diff < 1e-5, &format!("max |Δ| = {:.2e}", max_diff));
        }
    }
    ...
    {
        let num_matches = 50;
        let random = run_many_matches(&problem, &policy_random_schedule(&problem, 11), "random", num_matches, 999);
        let mdp = run_many_matches(&problem, &policy_mdp_vi(&problem).schedule, "mdp", num_matches, 999);
        let greedy = run_many_matches(&problem, &policy_greedy_hungarian(&problem, true), "greedy", num_matches, 999);
        println!("    random:  goal diff = {:.2} ± {:.2}", random.mean_goal_diff, random.sd_goal_diff);
        println!("    greedy:  goal diff = {:.2} ± {:.2}", greedy.mean_goal_diff, greedy.sd_goal_diff);
        println!("    MDP-VI:  goal diff = {:.2} ± {:.2}", mdp.mean_goal_diff, mdp.sd_goal_diff);
        ...
        c.check("MDP-VI > random in goal differential (Welch-t > 3)", t_mdp_vs_random > 3.0, &format!("t = {:.2}", t_mdp_vs_random));
        c.check("greedy > random in goal differential (Welch-t > 3)", t_greedy_vs_random > 3.0, &format!("t = {:.2}", t_greedy_vs_random));
        c.check("MDP-VI fairness OK in DES match", mdp.fairness_ok, "");
        c.check("greedy-Hungarian fairness OK in DES match", greedy.fairness_ok, "");
    }
