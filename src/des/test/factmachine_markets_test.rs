...
mod tests {
    ...
    fn daily_launch_cap_schedule_is_well_formed() {
        ...
        assert!(
            caps.iter().all(|&x| (2..=10).contains(&x)),
            "cap out of [2,10]: {caps:?}"
        );
    ...
    fn portfolio_run_breakdowns_are_consistent() {
        ...
        assert!(
            run.daily
                .iter()
                .filter(|d| d.day < horizon_days)
                .all(|d| d.opened <= d.market_cap),
        ...
        assert_eq!(
            total_by_kind, closed,
            "kind breakdown does not sum to closed markets"
        );
        assert_eq!(
            total_by_day, closed,
            "daily summaries do not sum to closed markets"
        );
        ...
        assert!(
            kind_markets(MarketKind::Binary) > 0,
            "run has no binary markets"
        );
        assert!(
            kind_markets(MarketKind::Scalar) > 0,
            "run has no scalar markets"
        );
